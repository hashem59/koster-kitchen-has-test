  ...
  var Drawer = function () {
    ...
    _createClass(Drawer, [{
      ...
      value: function open(event) {
        ...
        console.log("open drawer",this.element );
        if( this.element.getAttribute("data-section-id") == "sidebar-menu"){
          document.querySelector("#openSidebarMenuIcon").style.display = "none";
          document.querySelector("#closeSidebarMenuIcon").style.display = "block";
          document.querySelector(".AnnouncementBar").classList.add("index-high");
          document.querySelector(".shopify-section--header ").classList.add("index-high");
          this.pageOverlayElement.classList.add('notTop');
        }
        else{
          this.pageOverlayElement.classList.remove('notTop');
        }
      ...
      value: function close(event) {
        ...
        console.log("close drawer",this.element );
        if( this.element.getAttribute("data-section-id") == "sidebar-menu"){
          document.querySelector("#openSidebarMenuIcon").style.display = "block";
          document.querySelector("#closeSidebarMenuIcon").style.display = "none";
          document.querySelector(".AnnouncementBar").classList.remove("index-high");
          document.querySelector(".shopify-section--header ").classList.remove("index-high");
        }
        else{
          this.pageOverlayElement.classList.remove('notTop');
        }
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var ProductVariants = function () {
    function ProductVariants(container, options) {
      ...
      try {
        this.qty_discount_schema = JSON.parse(this.element.querySelector('[data-product-discount-rules-json]').innerHTML);
      } catch (e) {}
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _attachListeners() {
        ...
        this.delegateElement.on('click', '[data-action="mixmatch-add-to-cart"]', this._mixmatch_addToCart.bind(this));
        this.delegateElement.on('change', '[data-action="update-mixmatch-price"]', this._updateMixmatchPrice.bind(this));
        this.delegateElement.on('click', '[data-action="decrease-quantity"]', this._decreaseQuantity.bind(this));
        this.delegateElement.on('click', '[data-action="increase-quantity"]', this._increaseQuantity.bind(this));
        this.delegateElement.on('change', '[name="quantity"]', this._validateQuantity.bind(this));
        window.addEventListener('scroll', this._setStickyAddToCart.bind(this));

        var stickyAddToCart = document.querySelector('.sticky-btn-wrapper .add-to-cart');
        if (stickyAddToCart) {
      ...
      value: function _onVariantChanged(previousVariant, newVariant) {
        ...
        if (document.querySelector('[data-current-varaints]') && newVariant['id'] !== previousVariant['id'] ) {
          document.querySelector('[data-current-varaints]').innerHTML = newVariant['title']
        }
      ...
      key: '_setStickyAddToCart',
      value: function _updateProductPrices(newVariant, previousVariant) {
        var addToCartStickyBar = document.getElementsByClassName('sticky-btn-wrapper')[0];
        var productFrom = this.element.getElementsByClassName('ProductForm__Variants')[0];
        if ((productFrom.getBoundingClientRect()['y'] + productFrom.clientHeight) < 0 ) {
          addToCartStickyBar.classList.add('fixed-btn');
        } else {
          addToCartStickyBar.classList.remove('fixed-btn');
        }
      }

    }, {
      key: '_updateProductQtyPrices',
      value: function _updateProductQtyPrices() {
        if ((!document.querySelector('[data-sticky-addToCart-price]')) || document.querySelector('.MixMatch_List')) {
          return
        }
        var currntVariantCompare_at_price = 0;
        var productMetaPrices = this.element.querySelector('.ProductMeta__PriceList');
        var productQty = this.element.querySelector('.QuantitySelector__CurrentQuantity').value;
        if (this.element.querySelector('[data-product-set-discount]') && productQty >=2 ) {
          var currntVariantCompare_at_price = parseInt(document.querySelector('[data-sticky-addToCart-price]').getAttribute('data-varaint-price'));
          var currentVariantPrice = currntVariantCompare_at_price - 500;
        } else {
          var currentVariantPrice = parseInt(document.querySelector('[data-sticky-addToCart-price]').getAttribute('data-varaint-price'))
        }

        document.querySelector('[data-sticky-addToCart-price]').innerHTML =
         __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(currentVariantPrice*productQty, window.theme.moneyFormat);

        productMetaPrices.innerHTML = '';
        if (currntVariantCompare_at_price > currentVariantPrice) {
          productMetaPrices.innerHTML += '<span class="ProductMeta__Price Price Text--subdued u-h4" data-money-convertible>' + __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(currentVariantPrice*productQty, window.theme.moneyFormat) + '</span>';
          productMetaPrices.innerHTML += '<span class="ProductMeta__Price Price Price--compareAt Text--subdued u-h4" data-money-convertible>' + __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(currntVariantCompare_at_price*productQty, window.theme.moneyFormat) + '</span>';
        } else {
          productMetaPrices.innerHTML += '<span class="ProductMeta__Price Price Text--subdued u-h4" data-money-convertible>' + __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(currentVariantPrice*productQty, window.theme.moneyFormat) + '</span>';
        }
      ...
      value: function _updateProductPrices(newVariant, previousVariant) {
        ...
        if (!newVariant) {
          ...
          if (this.element.querySelector('[data-product-set-discount]') && productQty >=2 ) {
            var newVariantPrice = newVariant['price'] - 500;
            var newVariantCompare_at_price = newVariant['price'];
          } else {
            var newVariantPrice = newVariant['price'];
            var newVariantCompare_at_price = newVariant['compare_at_price'];
          }
          ...
          if (document.querySelector('[data-sticky-addToCart-price]')) {
            document.querySelector('[data-sticky-addToCart-price]').setAttribute('data-varaint-price', newVariant['price']);
            document.querySelector('[data-sticky-addToCart-price]').innerHTML =
             __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(newVariantPrice*productQty, window.theme.moneyFormat);
          }

          productMetaPrices.innerHTML = '';

          if (newVariantCompare_at_price > newVariantPrice) {
            productMetaPrices.innerHTML += '<span class="ProductMeta__Price Price Text--subdued u-h4" data-money-convertible>' + __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(newVariantPrice, window.theme.moneyFormat) + '</span>';
            productMetaPrices.innerHTML += '<span class="ProductMeta__Price Price Price--compareAt Text--subdued u-h4" data-money-convertible>' + __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(newVariantCompare_at_price, window.theme.moneyFormat) + '</span>';
          } else {
            productMetaPrices.innerHTML += '<span class="ProductMeta__Price Price Text--subdued u-h4" data-money-convertible>' + __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(newVariantPrice, window.theme.moneyFormat) + '</span>';
          }
      ...
      value: function _updateAddToCartButton(newVariant) {
        ...
        if (!newVariant) {
          ...
          document.body.classList.remove('disabled-add-to-cart');
      ...
      key: '_updateMixMatchAddToCartButton',
      value: function _updateMixMatchAddToCartButton(totalQtys, orderMin) {
        var addToCartButton = this.element.querySelector('.ProductForm__AddToCart');
        var minItemsErrorMsg = this.element.querySelector('.Alert--min-items');
        var pricePanel = this.element.querySelector('.TabPanel_Sticky');
        if (totalQtys < orderMin) {
          pricePanel.style.display = "none";
          addToCartButton.setAttribute('disabled', 'disabled');
          addToCartButton.removeAttribute('data-action');
          document.body.classList.add('disabled-add-to-cart');

          if (totalQtys >= 0 && orderMin > 1) {
            minItemsErrorMsg.style.display = "block"
          } else {
            minItemsErrorMsg.style.display = "none"
          }
        } else {
          minItemsErrorMsg.style.display = "none"
      ...
      key: '_updateMixmatchPrice',
      value: function _updateMixmatchPrice(event) {
        var mixMatchVariants = this.element.querySelectorAll('[max-match-variant]');
        if (!mixMatchVariants.length) {
          return;
        }
        function financial(x) {
          return Math.ceil(Number.parseFloat(x).toFixed(2));
        }
        var pricePanel = this.element.querySelector('.TabPanel_Sticky');
        var addToCartButton = this.element.querySelector('.ProductForm__AddToCart');
        var shotsPerBottle = parseInt(this.element.querySelector('[data-shoots-per-bottle]').getAttribute('data-shoots-per-bottle'));
        var orderMin = parseInt(this.element.querySelector('[data-order-min]').getAttribute('data-order-min'));
        var totalQtys = 0;
        var totalPrice = 0; // total discounted price
        var totalComparePrice = 0; // total original price
        var discountPercentage = 0; // quantity thesholds discounds
        var savedPrcentae = 0; // saved by the subscription (subscription discount)
        var variantsTitles = []
        mixMatchVariants.forEach(function (varEl) {
          var itemQty = parseInt(varEl.value);
          var itemPrice = parseInt(varEl.getAttribute('data-varaint-price'))
          //var itemComparePrice = parseInt(varEl.getAttribute('data-varaint-compare-price'))
          totalQtys += itemQty;
          totalComparePrice += itemQty*itemPrice;
          if (itemQty) { // if varaint is added get it's title
            variantsTitles.push(itemQty+'x ' + varEl.getAttribute('data-variant-title'));
          }
        });

        if (document.querySelector('[data-current-varaints]')) {
          document.querySelector('[data-current-varaints]').innerHTML = variantsTitles.join(', ');
        }

        this._updateMixMatchAddToCartButton(totalQtys, orderMin);
        if (totalQtys == 0) {
          return
        }

        if (this.qty_discount_schema) {
          if (totalQtys >= 1 && totalQtys < 19) {
            discountPercentage = this.qty_discount_schema[totalQtys];
          } else {
            discountPercentage = 29
          }
        }
        var itemUnits = this.element.querySelectorAll('[data-count]');
        var boxCountText = this.element.querySelector('.count-text');
        if (itemUnits) {
          itemUnits.forEach(function(count){
            try {
              var currCount = parseInt(count.getAttribute('data-count')) * totalQtys;
        ...
        if (boxCountText) {
          if (totalQtys === 1) {
            boxCountText.innerHTML = window.languages.moxMatchBox;
          } else {
            boxCountText.innerHTML = window.languages.moxMatchPluralBox;
          }
        }
        console.log('discountPercentage', discountPercentage);
        savedPrcentae = parseInt(this.element.querySelector('[data-saved-percentage]').getAttribute('data-saved-percentage'));

        var isSubscription = this.element.querySelector('[name="orderType"]').checked; // true if subscription selected
        // var discountPercentage = isSubscription ? discountPercentage+=savedPrcentae : discountPercentage;
        totalComparePrice = financial(totalComparePrice);
        var totalPrice = !isSubscription ? totalComparePrice : totalComparePrice - (totalComparePrice * (savedPrcentae/100));
        ...
        var pricePerShot = totalPrice/(totalQtys*shotsPerBottle)
        // console.log('savedPrcentae', savedPrcentae);
        // console.log('discountPercentage', discountPercentage);
        this.element.querySelector('[data-mixmatch-price]').innerHTML =
         __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(totalPrice, window.theme.moneyFormat);
         ...
         if (totalComparePrice === totalPrice) {
           this.element.querySelector('[data-maixmatch-compareAt-price]').style.display = "none"
         } else {
           this.element.querySelector('[data-maixmatch-compareAt-price]').style.display = "inline"
           this.element.querySelector('[data-maixmatch-compareAt-price]').innerHTML =
            __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(totalComparePrice, window.theme.moneyFormat);
         }

        this.element.querySelector('[data-shot-price]').innerHTML =
         __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(pricePerShot, window.theme.moneyFormat);

        this._mixmatchUpdateNextDiscount(totalQtys);
      }
    }, {
      key: '_mixmatchUpdateNextDiscount',
      value: function _mixmatchUpdatenextDiscount(totalQtys) {
        if (!this.qty_discount_schema) {
          this.element.querySelector('.Upsell_Panel').style.display = "none";
          return
        }
        var nextDiscount = this.qty_discount_schema[totalQtys+1];
        if (nextDiscount) {
          this.element.querySelectorAll('.YouCan_save').forEach(function(item){
            item.style.display = "";
          });
          this.element.querySelectorAll('[data-next-discount]').forEach(function(item){
            item.innerHTML = nextDiscount;
          })
        } else {
          this.element.querySelectorAll('.YouCan_save').forEach(function(item){
      ...
      key: '_mixmatch_addToCart',
      value: function _mixmatch_addToCart(event) {
        var _this7 = this;
        ...
        var orderType = this.element.querySelector('[name="orderType"]').checked; // true if subscription selected
        var sellingPlan = orderType ? this.element.querySelector('[name="selling_plan"]').value : '';
        // get mixmatch variants
        var mixMatchVariants = this.element.querySelectorAll('[max-match-variant]');
        // var formBody = JSON.stringify(__WEBPACK_IMPORTED_MODULE_2__helper_Form__["default"].serialize(formElement));
        var cartItems = [];
        var totalQtys = 0;
        ...
        mixMatchVariants.forEach(function (varEl) {
          ...
          if (itemQty) {
            var item = {
              'id': parseInt(varEl.getAttribute('data-variantID')),
              'quantity': itemQty,
              'selling_plan': sellingPlan
            };
            // add items o the begging so it show in the cart in the same order as on the mixmatch widget
        ...
        //   cartItems[index]['properties']['quantity'] = totalQtys
        // })
        var formData = {
         'items': cartItems
        };
      ...
      value: function _addToCart(event) {
        ...
        var mixMatchVariants = this.element.querySelector('[max-match-variant]');
        if (mixMatchVariants) {
          event.preventDefault(); // Prevent form to be submitted
          this._mixmatch_addToCart(event);
          return
        }
      ...
      value: function _decreaseQuantity(event, target, min) {
        var minVal = target.classList.contains('MixMatch_QtySelector') ? 0 : 1;
        target.nextElementSibling.value = Math.max(parseInt(target.nextElementSibling.value) - 1, minVal);
        this._updateMixmatchPrice();
        this._updateProductQtyPrices();
      }
      ...
      value: function _validateQuantity(event, target) {
        var minVal = target.nextElementSibling.classList.contains('MixMatch_QtySelector') ? 0 : 1;
        target.value = Math.max(parseInt(target.value) || minVal, minVal);
        this._updateMixmatchPrice();
        this._updateProductQtyPrices();
      }
    }]);
    ...
    _createClass(HeaderSection, [{
      ...
      //Close all menu items first
        ...
        fastdom.mutate(function () {
          ...
          console.log("target:", target, "add expanded class");
      ...
      value: function _closeMenu(event, target) {
        ...
        fastdom.mutate(function () {
          ...
          console.log("target:", target, "remove expanded class");
  ...
  var ProductRecommendationsSection = function () {
    ...
    _createClass(ProductRecommendationsSection, [{
      ...
      value: function _loadRecommendations() {
        ...
        return fetch(url).then(function (response) {
          return response.text().then(function (content) {
            ...
            var titleMaxH = 0;
            var priceMaxH = 0;
            var $prices = _this57.element.querySelectorAll('.ProductItem__PriceList--wrapper');
            _this57.element.querySelectorAll('.ProductItem__Title ').forEach(function (item, i){
              if (item.clientHeight > titleMaxH) {titleMaxH = item.clientHeight}
              if ($prices[i].clientHeight > priceMaxH) {priceMaxH = $prices[i].clientHeight}
            });
            document.documentElement.style.setProperty('--recommendations-title-height', titleMaxH + 'px');
            document.documentElement.style.setProperty('--recommendations-price-height', priceMaxH + 'px');
          });
        });
      }
    }, {
      ...
      value: function _createSlideshow() {
        if (this.element.querySelector('[data-flickity-config]')) {
  ...
  var ProductSection = function () {
    ...
    _createClass(ProductSection, [{
      ...
      value: function _switchTab(event, target) {
        ...
        // First, switch the current tab
        target.classList.add('is-active');
        __WEBPACK_IMPORTED_MODULE_7__helper_Dom__["default"].getSiblings(target, '.is-active').forEach(function (item) {
          item.classList.remove('is-active');
        });

        // Then, display the panel
        var tabPanelToShow = this.element.querySelectorAll('.' + target.getAttribute('aria-controls'));

        // tabPanelToShow.setAttribute('aria-hidden', 'false');
        tabPanelToShow.forEach(function (item) {
          item.setAttribute('aria-hidden', 'false');
          //document.querySelectorAll
          //console.log(__WEBPACK_IMPORTED_MODULE_7__helper_Dom__["default"].getSiblings(item, '.TabPanel[aria-hidden="false"]'));
          __WEBPACK_IMPORTED_MODULE_7__helper_Dom__["default"].getSiblings(item, '.TabPanel[aria-hidden="false"]').forEach(function (item_) {
            item_.setAttribute('aria-hidden', 'true');
          });
        });
  ...
  var ShopTheLookSection = function () {
    ...
    _createClass(ShopTheLookSection, [{
      ...
      value: function _createOuterCarousel() {
        ...
        console.log("outerCarousel",this.outerCarousel.flickityInstance);
  ...
  var SidebarMenuSection = function () {
    ...
    _createClass(SidebarMenuSection, [{
      ...
      value: function onSelect() {
        ...
        console.log("open sidebar drawer");
      }
    }, {
      key: 'onDeselect',
      value: function onDeselect() {
        this.sidebarDrawer.close();
        console.log("close sidebar drawer");
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var TestimonialsSection = function () {
    function TestimonialsSection(container) {
      ...
      this.element = container;
      console.log("testimonial: ", this.element);
      this.delegateElement = new domDelegate.Delegate(this.element);
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  (function () {
    ...
    (function () {
        if (document.querySelector('.community-Carousel')) {
          /* harmony import */var __WEBPACK_IMPORTED_MODULE_1__Carousel = __webpack_require__(2);
          var communityCarousel = new __WEBPACK_IMPORTED_MODULE_1__Carousel["default"](document.querySelector('.community-Carousel'));
